...
const DEFAULT_FILTER = {plumcoreAccess: true};
...
export default {
    ...
    actions: {
        async loadUsers({commit}, filter = {}) {
            let isEmpty = Object.keys(filter).length === 0;
            if (isEmpty) {
        ...
        async newUser({dispatch, state}, user) {
            user = Object.assign(DEFAULT_FILTER, user);
        ...
        async editUser({dispatch, commit, state}, user) {
            try {
                ...
                user = Object.assign(DEFAULT_FILTER, user);
                let response = await axios.post(`/api/user/update`, {user, filter});
        ...
        async loginLocalUser({commit, state}) {
            ...
            let filter = DEFAULT_FILTER;
            let response = await axios.post(`/api/user/check`, {id: user.id, filter});
        ...
        async loginUser({dispatch, commit}, {login, password}) {
            try {
                let filter = DEFAULT_FILTER;
                let response = await axios.post(`/api/user/login`, {login, password, filter});
                let isSuccess = response && response.data && response.data.user && response.data.user.id;
        ...
        async deleteUser({dispatch, state}, user) {
            let filter = DEFAULT_FILTER;
            await axios.post(`/api/user/delete`, {user, filter});
            return dispatch('loadUsers', state.filter);
        },
    },
    mutations: {
